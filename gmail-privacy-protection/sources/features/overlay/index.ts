...
import { Logger, Messenger, Storage, toMinutes } from '@/utilities';
...
export class Overlay extends EntryComponent {
  ...
  static async timer() {
    ...
    const isTimer = reactivate.kind === ReactivateKinds.Timer && reactivate.timer > 0;

    this.timeout = isTimer ? window.setTimeout(() => this.active(), reactivate.timer) : null;

    return enabled && isTimer ? reactivate.timer : null;
  }
  ...
  static async close() {
    ...
    if (time) {
      Logger.info(`プライバシー保護は${toMinutes(time)}分後に有効になります`);
    }
  }
